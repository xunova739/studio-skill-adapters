...
description: Codex version of the Studio workflow router. Use when the user mentions studio, autonomous mode, next step, full workflow, demand to launch, PRD.html, PRD.json after approval, agent development, verification, review, or deploy handoff in Codex/Codex Desktop. Project-local version using AGENTS.md and planning/status.json/.planning/status.json; does not install Claude hooks or depend on Claude model names/CronCreate.
---
...
- `prd_json`: 用户确认 `PRD.html` 后，再生成结构化 `PRD.json`。
...
`stage` 只能使用：`discovery`、`prd_html`、`prd_json`、`technical_plan`、`implementation`、`verification`、`review`、`deploy`、`done`。
